...
data <- read_tsv("vdjdb-2022-03-30/vdjdb_full.txt") %>%
  mutate(meta.structure.id = toupper(meta.structure.id)) %>%
  ...
  filter(count > 1 & has.structure)

modelled <- read_csv("VDJdb_Models_annotation.csv")

modelled <- with(modelled,
                 rbind(modelled %>%
                         mutate(gene = "TRB", cdr3 = 
                                  cdr3.beta) %>%
                         select(gene, cdr3),
                       modelled %>%
                         mutate(gene = "TRA", cdr3 = 
                                  cdr3.alpha) %>%
                         select(gene, cdr3))) %>%
  mutate(in_model = T) %>%
  unique
...
data.ga %>%
  ...
  geom_point(data = data.ga %>% filter(pdb_neigh), 
             color ="red", alpha = 0.5) +
  geom_point(data = data.ga %>% filter(in_model), 
             color ="black", shape = "+") +
